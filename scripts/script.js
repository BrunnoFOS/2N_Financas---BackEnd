    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (response) {
                ...
                document.getElementById('poupanca').textContent = formatarNumero(response.poupanca || 0);
            }
        }
    };
    xhr.send();
...
function enviarTransacaoParaBackend(transacao) {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'salvar_Transacao.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            const response = JSON.parse(xhr.responseText);
            document.getElementById('saldoTotal').textContent = formatarNumero(response.saldo_total);
            document.getElementById('saldoCaixa').textContent = formatarNumero(response.saldo_caixa);
            document.getElementById('poupanca').textContent = formatarNumero(response.poupanca);
        }
    };
    xhr.send(`descricao=${transacao.descricao}&valor=${transacao.valor}&tipo=${transacao.tipo}`);
}
...
function adicionarTransacao() {
    ...
    if (tipo === 'adicionarPoupanca' || tipo === 'retirarPoupanca') {
        ...
        enviarTransacaoParaBackend(transacao);
        return;
    }
